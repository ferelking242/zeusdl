        ...
        try:
            try:
                if sys.platform == 'win32':
                    ...
                    # Ref: https://github.com/ZeusDL/ZeusDL/issues/3124
...
def sanitize_filename(s, restricted=False, is_id=NO_DEFAULT):
    ...
    @param is_id        Whether this is an ID that should be kept unchanged if possible.
                        If unset, ZeusDL's new sanitization rules are in effect
    """
    if s == '':
        return ''
...
def bug_reports_message(before=';'):
    ...
    msg = (f'please report this issue on  https://github.com/{REPOSITORY}/issues?q= , '
           'filling out the appropriate issue template. Confirm you are on the latest version using  ZeusDL -U')
...
class ExtractorError(YoutubeDLError):
    ...
    def __init__(self, msg, tb=None, expected=False, cause=None, video_id=None, ie=None):
        ...
        If expected is set, this is a normal error message and most likely not a bug in ZeusDL.
        """
...
def write_string(s, out=None, encoding=None):
    ...
    enc, buffer = None, out
    # `mode` might be `None` (Ref: https://github.com/ZeusDL/ZeusDL/issues/8816)
...
def setproctitle(title):
    ...
    # Workaround for https://github.com/ZeusDL/ZeusDL/issues/4541
    try:
        import ctypes
    except ImportError:
        return
...
def _get_exe_version_output(exe, args, ignore_return_code=False):
    try:
        ...
        # SIGTTOU if ZeusDL is run in the background.
...
def ytdl_is_updateable():
    """ Returns if ZeusDL can be updated with -U """
...
def dfxp2srt(dfxp_data):
    ...
    # Fix UTF-8 encoded file wrongly marked as UTF-16. See https://github.com/ZeusDL/ZeusDL/issues/6543#issuecomment-1477169870
...
def write_xattr(path, key, value):
    ...
    elif getattr(xattr, '_zeusdl__identifier', None) == 'pyxattr':
    ...
    def load_configs(self):
        ...
        for location in opts.config_locations or []:
            ...
            if os.path.isdir(location):
                location = os.path.join(location, 'ZeusDL.conf')
...
class _UnsafeExtensionError(Exception):
    ...
    Ref: https://github.com/ZeusDL/ZeusDL/security/advisories/GHSA-79w7-vh3h-8g4j
    """
...
class FormatSorter:
    ...
    def _fill_sorting_fields(format):
        ...
        if format.get('preference') is None and format.get('ext') == 'flv' and re.match('[hx]265|he?vc?', format.get('vcodec') or ''):
            ...
            # ref. https://github.com/ZeusDL/ZeusDL/pull/5821
