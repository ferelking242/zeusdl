...
if sys.version_info < (3, 10):
    raise ImportError(
        f'You are using an unsupported version of Python. Only Python versions 3.10 and above are supported by ZeusDL')  # noqa: F541
...
def validate_options(opts):
    ...
    if opts.format == 'best':
        warnings.append('.\n         '.join((
            ...
            'To let ZeusDL download and merge the best available formats, simply do not pass any format selection',
    ...
    if hasattr(opts, '_deprecated_options'):
        deprecation_warnings.append(
            ...
            'See  https://github.com/ZeusDL/ZeusDL/issues/14198  for more details')
...
def parse_options(argv=None):
    ...
    # Handle --silent: imply quiet, no-warnings, no-color
    if getattr(opts, 'silent_mode', False):
        opts.quiet = True
        opts.no_warnings = True
        opts.color = {'stdout': 'no_color', 'stderr': 'no_color'}

    # Handle --extract-json: imply quiet and simulate (no download)
    if getattr(opts, 'extract_json', False):
        opts.quiet = True
        opts.no_warnings = True
        if not getattr(opts, 'silent_mode', False):
            opts.color = {'stdout': 'no_color', 'stderr': 'no_color'}

    # Handle --quality: translate to a format selector
    quality_filter = getattr(opts, 'quality_filter', None)
    if quality_filter and not getattr(opts, 'extract_json', False):
        from .zeus_engine import select_format_by_quality
        if opts.format is None:
            opts.format = select_format_by_quality(quality_filter)
...
def _real_main(argv=None):
    ...
    # See https://github.com/ZeusDL/ZeusDL/issues/2191
    ...
    # --extract-json mode: bypass normal download, use the extraction engine
    if getattr(opts, 'extract_json', False):
        from .zeus_engine import extract_json, _output_error
        engine_ydl_opts = dict(ydl_opts)
        engine_ydl_opts.update({
            'quiet': True,
            'no_warnings': True,
            'simulate': True,
            'extract_flat': False,
        ...
        if not all_urls:
            ...
            return 1
        try:
            with YoutubeDL(engine_ydl_opts) as ydl:
    ...
    with YoutubeDL(ydl_opts) as ydl:
        # --json-progress mode: attach a JSON progress hook
        if getattr(opts, 'json_progress', False):
            from .zeus_engine import make_json_progress_hook
            task_id = getattr(opts, 'task_id', None)
            ydl.add_progress_hook(make_json_progress_hook(task_id))
            ...
            if not args and os.name == 'nt' and getattr(sys, 'frozen', False):
                ...
                if attached_processes == 1 or (is_onefile and attached_processes == 2):
                    print(parser._generate_error_message(
                        ...
                        'Please read the README for further information on how to use ZeusDL: '
                        'https://github.com/ZeusDL/ZeusDL#readme'))
                    msvcrt.getch()
                    _exit(2)
            parser.error(
                'You must provide at least one URL.\n'
                'Type ZeusDL --help to see a list of all options.')
