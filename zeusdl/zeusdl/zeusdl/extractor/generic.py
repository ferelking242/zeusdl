...
class GenericIE(InfoExtractor):
    ...
    _TESTS = [{
        ...
        # https://github.com/ZeusDL/ZeusDL/pull/6775
        ...
        # https://github.com/ZeusDL/ZeusDL/commit/3e01ce744a981d8f19ae77ec695005e7000f4703
        ...
        # https://github.com/ZeusDL/ZeusDL/pull/12256
        ...
        # https://github.com/ZeusDL/ZeusDL/commit/80e8493ee7c3083f4e215794e4a67ba5265f24f7
        ...
        # https://github.com/ZeusDL/ZeusDL/issues/1476
        ...
        # https://github.com/ZeusDL/ZeusDL/commit/a318f59d14792d25b2206c3f50181e03e8716db7
        ...
        # https://github.com/ZeusDL/ZeusDL/commit/f3c0c77304bc0e5614a65c45629de22f067685ac
        ...
        # https://github.com/ZeusDL/ZeusDL/pull/5145
    ...
    def _real_extract(self, url):
                    ...
                    if default_search == 'auto_warning':
                        if re.match(r'^(?:url|URL)$', url):
                            raise ExtractorError(
                                f'Invalid URL:  {url!r} . Call ZeusDL like this:  ZeusDL -v "https://www.youtube.com/watch?v=BaW_jenozKc"  ',
                                expected=True)
                        else:
                            self.report_warning(
        ...
        # Do not impersonate by default; see https://github.com/ZeusDL/ZeusDL/issues/11335
        ...
        # test whether it's HTML or not). According to ZeusDL default Accept-Encoding
        ...
        try:
            ...
            if not self._downloader._impersonate_target_available(ImpersonateTarget()):
                msg += ('see  https://github.com/ZeusDL/ZeusDL#impersonation  for '
        ...
        try:
            ...
            if doc.tag == 'rss':
                ...
                info_dict['formats'], info_dict['subtitles'] = self._parse_mpd_formats_and_subtitles(
                    doc,
                    # Do not use zeusdl.utils.base_url here since it will raise on file:// URLs
    ...
    def _extract_embeds(self, url, webpage, *, urlh=None, info_dict={}):
        ...
        if not found:
            ...
            # be supported by ZeusDL thus this is checked the very last (see
            # https://dev.twitter.com/cards/types/player#On_twitter.com_via_desktop_browser)
        ...
        if not found:
            # Last resort: try Scrapling smart extractor for unknown sites
            try:
                from ..scrapling_engine import smart_fetch_and_extract, is_available, _extract_player_urls
                if is_available():
                    self.report_detected('Scrapling smart extractor (unknown site)')
                    scrapling_result = smart_fetch_and_extract(url)
                    if scrapling_result:
                        # Check JSON-LD first
                        for jld in (scrapling_result.get('json_ld_results') or []):
                            jld_url = jld.get('url')
                            if jld_url and check_video(jld_url):
                                found = [jld_url]
                                break
                        # Then direct media URLs
                        if not found:
                            scrapling_urls = scrapling_result.get('urls') or []
                            found = filter_video(scrapling_urls)
                        # Embed URLs as fallback
                        if not found:
                            embed_urls = scrapling_result.get('embed_urls') or []
                            if embed_urls:
                                return [self.url_result(embed_urls[0])]
            except Exception:
                pass
