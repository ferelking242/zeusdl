...
class XHamsterBaseListIE(InfoExtractor):
    """Shared helpers for XHamster list/page extractors."""

    _DOMAINS_RE = XHamsterIE._DOMAINS
    _BASE = 'https://xhamster.com'

    @staticmethod
    def _entry_from_thumb(video_url, video_id, title=None, thumb=None,
                          duration=None, view_count=None, uploader=None):
        """Return a uniform ZeusDL entry dict from scraped data."""
        return {
            '_type': 'url',
            'id': video_id,
            'url': video_url,
            ...
            'thumbnail': thumb,
            'duration': duration,
            'view_count': int_or_none(str_or_none(view_count or '').replace(',', '').replace(' ', '')),
            'uploader': uploader,
            'age_limit': 18,
        }

    def _scrape_video_list(self, start_url, list_id, label='page'):
        """Scrape a paginated XHamster video listing page by page."""
        next_url = start_url
        page_num = 0
        while next_url:
            page_num += 1
            ...
            for tag in re.findall(
                    r'(<a[^>]+class=["\'].*?\bvideo-thumb__image-container\b[^"\']*["\'][^>]*>)',
                    page):
                ...
                if not video_url or not XHamsterIE.suitable(video_url):
                    continue
                video_id = XHamsterIE._match_id(video_url)
                yield self._entry_from_thumb(video_url, video_id)

            mobj = re.search(r'<a[^>]+data-page=["\']next[^"\']*["\'][^>]*>', page)
            if not mobj:
                break
            next_attrs = extract_attributes(mobj.group(0))
            next_url = url_or_none(next_attrs.get('href'))


class XHamsterCategoryIE(XHamsterBaseListIE):
    IE_NAME = 'xhamster:category'
    IE_DESC = 'XHamster — category/tag page (all pages)'
    _VALID_URL = rf'https?://(?:[^/?#]+\.)?{XHamsterIE._DOMAINS}/(?:categories|tags)/(?P<id>[^/?#]+)'

    def _real_extract(self, url):
        cat_id = self._match_id(url)
        cat_url = f'{self._BASE}/categories/{cat_id}'
        webpage = self._download_webpage(cat_url, cat_id, impersonate=True)
        title = self._html_search_regex(
            r'<h1[^>]*>([^<]+)</h1>', webpage, 'title', default=cat_id)
        entries = self._scrape_video_list(cat_url, cat_id, label='category page')
        return self.playlist_result(entries, cat_id, title)


class XHamsterSearchIE(XHamsterBaseListIE):
    IE_NAME = 'xhamster:search'
    IE_DESC = 'XHamster — search results (all pages)'
    _VALID_URL = r'https?://(?:[^/?#]+\.)?xhamster\.(?:com|one|desi)/search/(?P<id>[^/?#]+)'

    def _real_extract(self, url):
        query = urllib.parse.unquote_plus(self._match_id(url))
        search_url = f'{self._BASE}/search/{urllib.parse.quote(query)}'
        entries = self._scrape_video_list(search_url, query, label='search page')
        return self.playlist_result(entries, query, f'XHamster — Search: {query}')


class XHamsterPlaylistIE(XHamsterBaseListIE):
    IE_NAME = 'xhamster:playlist'
    IE_DESC = 'XHamster — playlist/collection (all pages)'
    _VALID_URL = rf'https?://(?:[^/?#]+\.)?{XHamsterIE._DOMAINS}/playlists/(?P<id>[^/?#]+)'

    def _real_extract(self, url):
        playlist_id = self._match_id(url)
        webpage = self._download_webpage(url, playlist_id, impersonate=True)
        title = self._html_search_regex(
            r'<h1[^>]*>([^<]+)</h1>', webpage, 'title', default=f'Playlist {playlist_id}')
        entries = self._scrape_video_list(url, playlist_id, label='playlist page')
        return self.playlist_result(entries, playlist_id, title)


class XHamsterChannelIE(XHamsterBaseListIE):
    IE_NAME = 'xhamster:channel'
    IE_DESC = 'XHamster — official channel (all pages)'
    _VALID_URL = rf'https?://(?:[^/?#]+\.)?{XHamsterIE._DOMAINS}/channels/(?P<id>[^/?#]+)'

    def _real_extract(self, url):
        channel_id = self._match_id(url)
        channel_url = f'{self._BASE}/channels/{channel_id}/videos'
        webpage = self._download_webpage(channel_url, channel_id, impersonate=True)
        title = self._html_search_regex(
            r'<h1[^>]*>([^<]+)</h1>', webpage, 'title', default=channel_id)
