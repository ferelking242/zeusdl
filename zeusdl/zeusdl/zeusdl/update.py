...
UPDATE_SOURCES = {
    'stable': 'ZeusDL/ZeusDL',
    'nightly': 'ZeusDL/ZeusDL-nightly-builds',
    'master': 'ZeusDL/ZeusDL-master-builds',
}
...
def _get_variant_and_executable_path():
    ...
    if getattr(sys, 'frozen', False):
        ...
        # See: https://github.com/ZeusDL/ZeusDL/issues/11813
...
_NON_UPDATEABLE_REASONS = {
    ...
    **dict.fromkeys(
    ...
    'py2exe': 'py2exe is no longer supported by ZeusDL; This executable cannot be updated',
    'source': 'You cannot update when running from source code; Use git to pull the latest changes',
    'unknown': 'You installed ZeusDL from a manual build or with a package manager; Use that to update',
    'other': 'You are using an unofficial build of ZeusDL; Build the executable again',
}
...
def _get_binary_name():
    return format_field(_FILE_SUFFIXES, detect_variant(), template='ZeusDL%s', ignore=None, default=None)
...
def _get_outdated_warning():
    # Only ZeusDL guarantees a stable release at least every 90 days
    if not ORIGIN.startswith('ZeusDL/'):
        return None
    ...
    with contextlib.suppress(Exception):
        ...
        if last_updated < dt.datetime.now(dt.timezone.utc).date() - dt.timedelta(days=90):
            return ('\n         '.join((
                f'Your ZeusDL version ({__version__}) is older than 90 days!',
                'It is strongly recommended to always use the latest version.',
                f'{is_non_updateable() or """Run "ZeusDL --update" or "ZeusDL -U" to update"""}.',
...
class Updater:
    ...
    def __init__(self, ydl, target: str | None = None):
        ...
        if '/' in self.requested_channel:
            ...
            if not self.requested_repo.startswith('ZeusDL/') and self.requested_repo != self._origin:
    ...
    def _process_update_spec(self, lockfile: str, resolved_tag: str):
        ...
        for line in lines:
            ...
            if re.match(pattern, self._identifier):
                ...
                self._report_error(
                    f'ZeusDL cannot be updated to {resolved_tag} since you are on an older Python version '
    ...
    def update(self, update_info=NO_DEFAULT):
        """Update ZeusDL executable to the latest version
        ...
        self.ydl.to_screen(f'Updated ZeusDL to {update_label}')
        return True
    ...
    def _block_restart(self, msg):
        def wrapper():
            self._report_error(f'{msg}. Restart ZeusDL to use the updated version', expected=True)
...
def run_update(ydl):
    ...
    """
    deprecation_warning(
        '"zeusdl.update.run_update(ydl)" is deprecated and may be removed in a future version. '
        'Use "zeusdl.update.Updater(ydl).update()" instead')
