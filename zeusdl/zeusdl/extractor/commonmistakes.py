...
class CommonMistakesIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        msg = (
            f'You\'ve asked ZeusDL to download the URL "{url}". '
            'That doesn\'t make any sense. '
            'Simply remove the parameter in your command or configuration.'
        )
        if not self.get_param('verbose'):
            msg += ' Add -v to the command line to see what arguments and configuration ZeusDL has'
...
class BlobIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        raise ExtractorError(
            'You\'ve asked ZeusDL to download a blob URL. '
            'A blob URL exists only locally in your browser. '
            'It is not possible for ZeusDL to access it.', expected=True)
