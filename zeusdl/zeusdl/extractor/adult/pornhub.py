...
class PornHubPagedVideoListIE(PornHubPagedPlaylistBaseIE):
    ...
    def suitable(cls, url):
        return (False
                if PornHubIE.suitable(url) or PornHubUserIE.suitable(url)
                or PornHubUserVideosUploadIE.suitable(url)
                or PornHubShortiesIE.suitable(url) or PornHubAlbumIE.suitable(url)
...
class PornHubShortiesIE(PornHubBaseIE):
    """Extract a single PornHub Shortie (short-form vertical video)."""
    IE_NAME = 'pornhub:shorties'
    _VALID_URL = rf'https?://(?:[a-zA-Z0-9.-]+\.)?{PornHubBaseIE._PORNHUB_HOST_RE}/shorties/(?P<id>[\da-z]+)'
    ...
    def _real_extract(self, url):
        ...
        # Direct MP4 URL: appears inside the thumbnailUrl as the base before /plain/ params
        mp4_base = self._search_regex(
            r'(https?://[^\s"\'<>]+/original_\d+\.mp4)',
            webpage, 'mp4 url', default=None)

        # Title: try OG then page <title>
        title = (
        ...
        # Tags from page
        tags = list(orderedSet(re.findall(
            r'class="[^"]*categoriesTag[^"]*"[^>]*>[^<]*<a[^>]*>([^<]+)', webpage)))
        if not tags:
            tags = list(orderedSet(re.findall(r'/categories/([^"?/]+)', webpage)))

        # Thumbnail: strip the image-processing suffix from the URL
        thumbnail = json_ld.get('thumbnail') or ''
        if '/plain/' in thumbnail:
            thumbnail = thumbnail.split('/plain/')[0]
        if not thumbnail:
        ...
        uploader = (
            json_ld.get('uploader')
            or self._html_search_regex(
                r'class="[^"]*username[^"]*"[^>]*><a[^>]*>([^<]+)', webpage, 'uploader', default=None)
        )
        ...
        return {
            ...
            'age_limit': 18,
            'webpage_url': url,
            'extractor': 'PornHubShorties',
            'extractor_key': 'PornHubShorties',
        }


class PornHubAlbumIE(PornHubBaseIE):
    """Extract photos from a PornHub photo album."""
    IE_NAME = 'pornhub:album'
    _VALID_URL = rf'https?://(?:[a-zA-Z0-9.-]+\.)?{PornHubBaseIE._PORNHUB_HOST_RE}/album/(?P<id>\d+)'
    ...
    def _real_extract(self, url):
        ...
        uploader = self._html_search_regex(
            r'class="[^"]*usernameBadgesWrapper[^"]*"[^>]*>.*?<a[^>]+href="/users/([^"]+)"',
            webpage, 'uploader', default=None, flags=re.DOTALL)
        if not uploader:
            uploader = self._html_search_regex(
                r'<a[^>]+class="[^"]*bolded[^"]*"[^>]*>([^<]+)</a>', webpage, 'uploader', default=None)

        # Photo IDs from /photo/XXXXX links
        photo_ids = list(orderedSet(re.findall(r'/photo/(\d+)', webpage)))

        entries = []
        for photo_id in photo_ids:
            photo_url = f'https://www.{host}/photo/{photo_id}'
            entries.append(self.url_result(photo_url, video_id=photo_id, video_title=f'Photo {photo_id}'))
        ...
        return self.playlist_result(
            ...
            playlist_title=title.strip() if title else None,
