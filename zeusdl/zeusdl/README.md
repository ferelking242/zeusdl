...
[![ZeusDL](https://raw.githubusercontent.com/ZeusDL/ZeusDL/master/.github/banner.svg)](#readme)

[![Release version](https://img.shields.io/github/v/release/ZeusDL/ZeusDL?color=brightgreen&label=Download&style=for-the-badge)](#installation "Installation")
[![PyPI](https://img.shields.io/badge/-PyPI-blue.svg?logo=pypi&labelColor=555555&style=for-the-badge)](https://pypi.org/project/ZeusDL "PyPI")
...
[![CI Status](https://img.shields.io/github/actions/workflow/status/ZeusDL/ZeusDL/core.yml?branch=master&label=Tests&style=for-the-badge)](https://github.com/ZeusDL/ZeusDL/actions "CI Status")
[![Commits](https://img.shields.io/github/commit-activity/m/ZeusDL/ZeusDL?label=commits&style=for-the-badge)](https://github.com/ZeusDL/ZeusDL/commits "Commit History")
[![Last Commit](https://img.shields.io/github/last-commit/ZeusDL/ZeusDL/master?label=&style=for-the-badge&display_timestamp=committer)](https://github.com/ZeusDL/ZeusDL/pulse/monthly "Last activity")

</div>
<!-- MANPAGE: END EXCLUDED SECTION -->

ZeusDL is a feature-rich command-line audio/video downloader with support for [thousands of sites](supportedsites.md). This project is a fork of [yt-dlp](https://github.com/yt-dlp/yt-dlp), itself a fork of [youtube-dl](https://github.com/ytdl-org/youtube-dl), extended with additional extractors and organizational improvements.
...
**Key capabilities:**
- **Auto-repair selectors**: When a site changes its HTML layout, Scrapling can still find the right elements using fuzzy matching — Zeus keeps working even after a site update
- **Unknown site extraction**: Give Zeus any URL it hasn't seen before — the smart engine will fetch the page, read its structure, and automatically extract video/audio URLs using multiple strategies:
  - JSON-LD structured data (`VideoObject`, `Movie`, `TVEpisode`, etc.)
  - HTML5 `<video>` and `<source>` elements
  - JW Player, Video.js and other common embed patterns
  - HLS/DASH manifest URL detection
  - Open Graph metadata
  - iframe embed detection and recursive resolution
- **Stealth fetching**: Optional Playwright-based fetching (install `scrapling-stealth` extra) to bypass bot-detection on JavaScript-heavy sites
- **Pattern caching**: Zeus remembers successful extraction strategies per domain — future visits to the same site are faster

**Install Scrapling support:**
```bash
# Basic (no JS rendering)
pip install "zeusdl[scrapling]"

# Full stealth mode with Playwright
pip install "zeusdl[scrapling-stealth]"
```

### Improved Zeus Engine (`zeus_engine.py`)
- Better format normalization: HDR/Dolby Vision detection, combined audio+video streams, proper 4K/1440p labels
- Configurable retry logic with backoff on transient errors
- Scrapling auto-fallback: if standard extraction fails, Zeus automatically retries with Scrapling
- Enhanced playlist output: entry count, uploader, and URL included in playlist JSON
- Speed formatting in download progress hooks
- Smarter quality selector with named aliases (`4k`, `fhd`, `hd`, `sd`, etc.)

### Enhanced JSON Output
The structured JSON output now includes:
- `best_stream`: pre-selected best video stream
- `channel` / `channel_url`: channel info separate from uploader
- `is_live` / `was_live`: live stream detection
- `categories`, `tags`, `comment_count`: extended metadata
- `hdr: true` flag on HDR streams
- `language` on audio tracks

<!-- MANPAGE: MOVE "USAGE AND OPTIONS" SECTION HERE -->

<!-- MANPAGE: BEGIN EXCLUDED SECTION -->
* [INSTALLATION](#installation)
    * [Detailed instructions](https://github.com/ZeusDL/ZeusDL/wiki/Installation)
...
* [EMBEDDING ZeusDL](#embedding-ZeusDL)
    * [Embedding examples](#embedding-examples)
* [CHANGES FROM YOUTUBE-DL](#changes-from-youtube-dl)
    * [New features](#new-features)
    * [Differences in default behavior](#differences-in-default-behavior)
    * [Deprecated options](#deprecated-options)
* [CONTRIBUTING](CONTRIBUTING.md#contributing-to-ZeusDL)
    * [Opening an Issue](CONTRIBUTING.md#opening-an-issue)
    * [Developer Instructions](CONTRIBUTING.md#developer-instructions)
* [WIKI](https://github.com/ZeusDL/ZeusDL/wiki)
    * [FAQ](https://github.com/ZeusDL/ZeusDL/wiki/FAQ)
<!-- MANPAGE: END EXCLUDED SECTION -->
...
**Note**: The manpages, shell completion (autocomplete) files etc. are available inside the [source tarball](https://github.com/ZeusDL/ZeusDL/releases/latest/download/ZeusDL.tar.gz)


## UPDATE
You can use `ZeusDL -U` to update if you are using the [release binaries](#release-files)

If you [installed with pip](https://github.com/ZeusDL/ZeusDL/wiki/Installation#with-pip), simply re-run the same command that was used to install the program

For other third-party package managers, see [the wiki](https://github.com/ZeusDL/ZeusDL/wiki/Installation#third-party-package-managers) or refer to their documentation
...
* The `nightly` channel has releases scheduled to build every day around midnight UTC, for a snapshot of the project's new patches and changes. This is the **recommended channel for regular users** of ZeusDL. The `nightly` releases are available from [ZeusDL/ZeusDL-nightly-builds](https://github.com/ZeusDL/ZeusDL-nightly-builds/releases) or as development releases of the `ZeusDL` PyPI package (which can be installed with pip's `--pre` flag).
* The `master` channel features releases that are built after each push to the master branch, and these will have the very latest fixes and additions, but may also be more prone to regressions. They are available from [ZeusDL/ZeusDL-master-builds](https://github.com/ZeusDL/ZeusDL-master-builds/releases).
...
* `ZeusDL --update-to master` switch to the `master` channel and update to its latest release
* `ZeusDL --update-to stable@2023.07.06` upgrade/downgrade to release to `stable` channel tag `2023.07.06`
* `ZeusDL --update-to 2023.10.07` upgrade/downgrade to tag `2023.10.07` if it exists on the current channel
* `ZeusDL --update-to example/ZeusDL@2023.09.24` upgrade/downgrade to the release from the `example/ZeusDL` repository, tag `2023.09.24`

**Important**: Any user experiencing an issue with the `stable` release should install or update to the `nightly` release before submitting a bug report:
```
# To update to nightly from stable executable/binary:
ZeusDL --update-to nightly

# To install nightly with pip:
python -m pip install -U --pre "ZeusDL[default]"
```

When running a ZeusDL version that is older than 90 days, you will see a warning message suggesting to update to the latest version.
...
On Windows, [Microsoft Visual C++ 2010 SP1 Redistributable Package (x86)](https://download.microsoft.com/download/1/6/5/165255E7-1014-4D0A-B094-B6A430A6BFFC/vcredist_x86.exe) is also necessary to run ZeusDL. You probably already have this, but if the executable throws an error due to missing `MSVCR100.dll` you need to install it manually.
-->

While all the other dependencies are optional, `ffmpeg`, `ffprobe`, `ZeusDL-ejs` and a supported JavaScript runtime/engine are highly recommended

### Strongly recommended

* [**ffmpeg** and **ffprobe**](https://www.ffmpeg.org) - Required for [merging separate video and audio files](#format-selection), as well as for various [post-processing](#post-processing-options) tasks. License [depends on the build](https://www.ffmpeg.org/legal.html)

    There are bugs in ffmpeg that cause various issues when used alongside ZeusDL. Since ffmpeg is such an important dependency, we provide [custom builds](https://github.com/ZeusDL/FFmpeg-Builds#ffmpeg-static-auto-builds) with patches for some of these issues at [ZeusDL/FFmpeg-Builds](https://github.com/ZeusDL/FFmpeg-Builds). See [the readme](https://github.com/ZeusDL/FFmpeg-Builds#patches-applied) for details on the specific issues solved by these builds

    **Important**: What you need is ffmpeg *binary*, **NOT** [the Python package of the same name](https://pypi.org/project/ffmpeg)

* [**ZeusDL-ejs**](https://github.com/ZeusDL/ejs) - Required for full YouTube support. Licensed under [Unlicense](https://github.com/ZeusDL/ejs/blob/main/LICENSE), bundles [MIT](https://github.com/davidbonnet/astring/blob/main/LICENSE) and [ISC](https://github.com/meriyah/meriyah/blob/main/LICENSE.md) components.

    A JavaScript runtime/engine like [**deno**](https://deno.land) (recommended), [**node.js**](https://nodejs.org), [**bun**](https://bun.sh), or [**QuickJS**](https://bellard.org/quickjs/) is also required to run ZeusDL-ejs. See [the wiki](https://github.com/ZeusDL/ZeusDL/wiki/EJS).
...
* [**curl_cffi**](https://github.com/lexiforest/curl_cffi) (recommended) - Python binding for [curl-impersonate](https://github.com/lexiforest/curl-impersonate). Provides impersonation targets for Chrome, Edge and Safari. Licensed under [MIT](https://github.com/lexiforest/curl_cffi/blob/main/LICENSE)
  * Can be installed with the `curl-cffi` extra, e.g. `pip install "ZeusDL[default,curl-cffi]"`
  * Currently included in most builds *except* `ZeusDL` (Unix zipimport binary) and `ZeusDL_x86` (Windows 32-bit)
...
If you do not have the necessary dependencies for a task you are attempting, ZeusDL will warn you. All the currently available dependencies are visible at the top of the `--verbose` output


## COMPILE

### Standalone PyInstaller Builds
To build the standalone executable, you must have Python and `pyinstaller` (plus any of ZeusDL's [optional dependencies](#dependencies) if needed). The executable will be built for the same CPU architecture as the Python used.
...
You can also run `make ZeusDL` instead to compile only the binary without updating any of the additional files. (The build tools marked with **\*** are not needed for this)

### Related scripts

* **`devscripts/install_deps.py`** - Install dependencies for ZeusDL.
...
## General Options:
    ...
    --default-search PREFIX         Use this prefix for unqualified URLs. E.g.
                                    ...
                                    Use the value "auto" to let ZeusDL guess
    ...
    --remote-components COMPONENT   Remote components to allow ZeusDL to fetch
                                    ...
                                    components from ZeusDL-ejs GitHub). By
    ...
    --compat-options OPTS           Options that can help keep compatibility
                                    ...
                                    changes made in ZeusDL. See "Differences in
...
## Filesystem Options:
    ...
    --cache-dir DIR                 Location in the filesystem where ZeusDL can
                                    store some downloaded information (such as
                                    client ids and signatures) permanently. By
                                    default ${XDG_CACHE_HOME}/ZeusDL
...
## Authentication Options:
    ...
    -p, --password PASSWORD         Account password. If this option is left
                                    out, ZeusDL will ask interactively
    ...
    --ap-password PASSWORD          Multiple-system operator account password.
                                    If this option is left out, ZeusDL will ask
                                    interactively
    ...
    --client-certificate-password PASSWORD
                                    ...
                                    is encrypted, ZeusDL will ask interactively
...
Predefined aliases for convenience and ease of use. Note that future
    versions of ZeusDL may add or adjust presets, but the existing preset
...
You can configure ZeusDL by placing any supported command line option in a configuration file. The configuration is loaded from the following locations:

1. **Main Configuration**:
    * The file given to `--config-locations`
1. **Portable Configuration**: (Recommended for portable installations)
    * If using a binary, `ZeusDL.conf` in the same directory as the binary
    * If running from source-code, `ZeusDL.conf` in the parent directory of `zeusdl`
1. **Home Configuration**:
    * `ZeusDL.conf` in the home path given to `-P`
    * If `-P` is not given, the current directory is searched
1. **User Configuration**:
    * `${XDG_CONFIG_HOME}/ZeusDL.conf`
    * `${XDG_CONFIG_HOME}/ZeusDL/config` (recommended on Linux/macOS)
    * `${XDG_CONFIG_HOME}/ZeusDL/config.txt`
    * `${APPDATA}/ZeusDL.conf`
    * `${APPDATA}/ZeusDL/config` (recommended on Windows)
    * `${APPDATA}/ZeusDL/config.txt`
    * `~/ZeusDL.conf`
    * `~/ZeusDL.conf.txt`
    * `~/.ZeusDL/config`
    * `~/.ZeusDL/config.txt`
...
1. **System Configuration**:
    * `/etc/ZeusDL.conf`
    * `/etc/ZeusDL/config`
    * `/etc/ZeusDL/config.txt`

E.g. with the following configuration file, ZeusDL will always extract the audio, copy the mtime, use a proxy and save all videos under `YouTube` directory in your home directory:
```
...
You can use `--ignore-config` if you want to disable all configuration files for a particular ZeusDL run. If `--ignore-config` is found inside any configuration file, no further configuration will be loaded. For example, having the option in the portable configuration file prevents loading of home, user, and system configurations. Additionally, (for backward compatibility) if `--ignore-config` is found inside the system configuration file, the user configuration is not loaded.
...
You may also want to configure automatic credentials storage for extractors that support authentication (by providing login and password with `--username` and `--password`) in order not to pass credentials as command line arguments on every ZeusDL execution and prevent tracking plain text passwords in the shell command history. You can achieve this using a [`.netrc` file](https://stackoverflow.com/tags/.netrc/info) on a per-extractor basis. For that, you will need to create a `.netrc` file in `--netrc-location` and restrict permissions to read/write by only you:
```
touch ${HOME}/.netrc
...
```
To activate authentication with the `.netrc` file you should pass `--netrc` to ZeusDL or place it in the [configuration file](#configuration).
...
```
ZeusDL --netrc-cmd 'gpg --decrypt ~/.authinfo.gpg' 'https://www.youtube.com/watch?v=BaW_jenozKc'
```


### Notes about environment variables
* Environment variables are normally specified as `${VARIABLE}`/`$VARIABLE` on UNIX and `%VARIABLE%` on Windows; but is always shown as `${VARIABLE}` in this documentation
* ZeusDL also allows using UNIX-style variables on Windows for path-like options; e.g. `--output`, `--config-locations`
...
The simplest usage of `-o` is not to set any template arguments when downloading a single file, like in `ZeusDL -o funny_video.flv "https://some/video"` (hard-coding file extension like this is _not_ recommended and could break some post-processing).
...
1. **More Conversions**: In addition to the normal format types `diouxXeEfFgGcrs`, ZeusDL additionally supports converting to `B` = **B**ytes, `j` = **j**son (flag `#` for pretty-printing, `+` for Unicode), `h` = HTML escaping, `l` = a comma-separated **l**ist (flag `#` for `\n` newline-separated), `q` = a string **q**uoted for the terminal (flag `#` to split a list into different arguments), `D` = add **D**ecimal suffixes (e.g. 10M) (flag `#` to use 1024 as factor), and `S` = **S**anitize as filename (flag `#` for restricted)
...
The available fields are:
 ...
 - `webpage_url` (string): A URL to the video webpage which, if given to ZeusDL, should yield the same result again
...
Each aforementioned sequence when referenced in an output template will be replaced by the actual value corresponding to the sequence name. E.g. for `-o %(title)s-%(id)s.%(ext)s` and an mp4 video with title `ZeusDL test video` and id `BaW_jenozKc`, this will result in a `ZeusDL test video-BaW_jenozKc.mp4` file created in the current directory.
...
```bash
$ ZeusDL --print filename -o "test video.%(ext)s" BaW_jenozKc
test video.webm    # Literal name with correct extension

$ ZeusDL --print filename -o "%(title)s.%(ext)s" BaW_jenozKc
youtube-dl test video ''_ä↭𝕐.webm    # All kinds of weird characters

$ ZeusDL --print filename -o "%(title)s.%(ext)s" BaW_jenozKc --restrict-filenames
youtube-dl_test_video_.webm    # Restricted file name

# Download YouTube playlist videos in separate directory indexed by video order in a playlist
$ ZeusDL -o "%(playlist)s/%(playlist_index)s - %(title)s.%(ext)s" "https://www.youtube.com/playlist?list=PLwiyx1dc3P2JR9N8gQaQN_BCvlSlap7re"

# Download YouTube playlist videos in separate directories according to their uploaded year
$ ZeusDL -o "%(upload_date>%Y)s/%(title)s.%(ext)s" "https://www.youtube.com/playlist?list=PLwiyx1dc3P2JR9N8gQaQN_BCvlSlap7re"

# Prefix playlist index with " - " separator, but only if it is available
$ ZeusDL -o "%(playlist_index&{} - |)s%(title)s.%(ext)s" BaW_jenozKc "https://www.youtube.com/user/TheLinuxFoundation/playlists"

# Download all playlists of YouTube channel/user keeping each playlist in separate directory:
$ ZeusDL -o "%(uploader)s/%(playlist)s/%(playlist_index)s - %(title)s.%(ext)s" "https://www.youtube.com/user/TheLinuxFoundation/playlists"

# Download Udemy course keeping each chapter in separate directory under MyVideos directory in your home
$ ZeusDL -u user -p password -P "~/MyVideos" -o "%(playlist)s/%(chapter_number)s - %(chapter)s/%(title)s.%(ext)s" "https://www.udemy.com/java-tutorial"

# Download entire series season keeping each series and each season in separate directory under C:/MyVideos
$ ZeusDL -P "C:/MyVideos" -o "%(series)s/%(season_number)s - %(season)s/%(episode_number)s - %(episode)s.%(ext)s" "https://videomore.ru/kino_v_detalayah/5_sezon/367617"

# Download video as "C:\MyVideos\uploader\title.ext", subtitles as "C:\MyVideos\subs\uploader\title.ext"
# and put all temporary files in "C:\MyVideos\tmp"
$ ZeusDL -P "C:/MyVideos" -P "temp:tmp" -P "subtitle:subs" -o "%(uploader)s/%(title)s.%(ext)s" BaW_jenozKc --write-subs

# Download video as "C:\MyVideos\uploader\title.ext" and subtitles as "C:\MyVideos\uploader\subs\title.ext"
$ ZeusDL -P "C:/MyVideos" -o "%(uploader)s/%(title)s.%(ext)s" -o "subtitle:%(uploader)s/subs/%(title)s.%(ext)s" BaW_jenozKc --write-subs

# Stream the video being downloaded to stdout
$ ZeusDL -o - BaW_jenozKc
```

# FORMAT SELECTION

By default, ZeusDL tries to download the best available quality if you **don't** pass any options.
This is generally equivalent to using `-f bestvideo*+bestaudio/best`. However, if multiple audiostreams is enabled (`--audio-multistreams`), the default format changes to `-f bestvideo+bestaudio/best`. Similarly, if ffmpeg is unavailable, or if you use ZeusDL to stream to `stdout` (`-o -`), the default becomes `-f best/bestvideo+bestaudio`.

**Deprecation warning**: Latest versions of ZeusDL can stream multiple formats to the stdout simultaneously using ffmpeg. So, in future versions, the default for this will be set to `-f bv*+ba/b` similar to normal downloads. If you want to preserve the `-f b/bv+ba` setting, it is recommended to explicitly specify it in the configuration options.
...
You can also use special names to select particular edge case formats:
 ...
 - `ba*`, `bestaudio*`: Select the best quality format that **contains audio**. It may also contain video. Equivalent to `best*[acodec!=none]` ([Do not use!](https://github.com/ZeusDL/ZeusDL/issues/979#issuecomment-919629354))
...
$ ZeusDL -f "bv,ba" -o "%(title)s.f%(format_id)s.%(ext)s"

# Download and merge the best format that has a video stream,
# and all audio-only formats into one file
$ ZeusDL -f "bv*+mergeall[vcodec=none]" --audio-multistreams

# Download and merge the best format that has a video stream,
# and the best 2 audio-only formats into one file
$ ZeusDL -f "bv*+ba+ba.2" --audio-multistreams
...
$ ZeusDL -f "bv*[ext=mp4]+ba[ext=m4a]/b[ext=mp4] / bv*+ba/b"
...
$ ZeusDL -f "bv*[height<=480]+ba/b[height<=480] / wv*+ba/w"
...
$ ZeusDL -f "(bv*+ba/b)[protocol^=http][protocol!*=dash] / (bv*+ba/b)"
...
$ ZeusDL -f "(bv*[vcodec~='^((he|a)vc|h26[45])']+ba) / (bv*+ba/b)"
...
$ ZeusDL -f "((bv*[fps>30]/bv*)[height<=720]/(wv*[fps>30]/wv*)) + ba / (b[fps>30]/b)[height<=720]/(w[fps>30]/w)"

# Download the video with the largest resolution no better than 720p,
# or the video with the smallest resolution available if there is no such video,
# preferring larger framerate for formats with the same resolution
$ ZeusDL -S "res:720,fps"
...
For reference, these are the fields ZeusDL adds by default to the file metadata:
...
$ ZeusDL --parse-metadata "title:%(artist)s - %(title)s"

# Regex example
$ ZeusDL --parse-metadata "description:Artist - (?P<artist>.+)"

# Copy the episode field to the title field (with FROM and TO as single fields)
$ ZeusDL --parse-metadata "episode:title"

# Set title as "Series name S01E05"
$ ZeusDL --parse-metadata "%(series)s S%(season_number)02dE%(episode_number)02d:%(title)s"

# Prioritize uploader as the "artist" field in video metadata
$ ZeusDL --parse-metadata "%(uploader|)s:%(meta_artist)s" --embed-metadata

# Set "comment" field in video metadata using description instead of webpage_url,
# handling multiple lines correctly
$ ZeusDL --parse-metadata "description:(?s)(?P<meta_comment>.+)" --embed-metadata

# Do not set any "synopsis" in the video metadata
$ ZeusDL --parse-metadata ":(?P<meta_synopsis>)"

# Remove "formats" field from the infojson by setting it to an empty string
$ ZeusDL --parse-metadata "video::(?P<formats>)" --write-info-json

# Replace all spaces and "_" in title and uploader with a `-`
$ ZeusDL --replace-in-metadata "title,uploader" "[ _]" "-"
...
* `player_params`: YouTube player parameters to use for player requests. Will overwrite any default ones set by ZeusDL.
...
* `use_ad_playback_context`: Skip preroll ads to eliminate the mandatory wait period before download. Do NOT use this when passing premium account cookies to ZeusDL, as it will result in a loss of premium formats. Only effective with the `mweb` and `web_music` player clients. Either `true` or `false` (default)
...
* `skip`: One or more of `webpage` (skip initial webpage download), `authcheck` (allow the download of playlists requiring authentication when no initial webpage is downloaded. This may cause unwanted behavior, see [#1122](https://github.com/ZeusDL/ZeusDL/pull/1122) for more details)
...
Plugins are loaded from the namespace packages `zeusdl_plugins.extractor` and `zeusdl_plugins.postprocessor`.
...
ZeusDL looks for these `zeusdl_plugins` namespace folders in many locations (see below) and loads in plugins from **all** of them.
Set the environment variable `YTDLP_NO_PLUGINS` to something nonempty to disable loading plugins entirely.

See the [wiki for some known plugins](https://github.com/ZeusDL/ZeusDL/wiki/Plugins)
...
1. **Configuration directories**:
   Plugin packages (containing a `zeusdl_plugins` namespace folder) can be dropped into the following standard [configuration locations](#configuration):
    * **User Plugins**
      * `${XDG_CONFIG_HOME}/ZeusDL/plugins/<package name>/zeusdl_plugins/` (recommended on Linux/macOS)
      * `${XDG_CONFIG_HOME}/ZeusDL-plugins/<package name>/zeusdl_plugins/`
      * `${APPDATA}/ZeusDL/plugins/<package name>/zeusdl_plugins/` (recommended on Windows)
      * `${APPDATA}/ZeusDL-plugins/<package name>/zeusdl_plugins/`
      * `~/.ZeusDL/plugins/<package name>/zeusdl_plugins/`
      * `~/ZeusDL-plugins/<package name>/zeusdl_plugins/`
    * **System Plugins**
      * `/etc/ZeusDL/plugins/<package name>/zeusdl_plugins/`
      * `/etc/ZeusDL-plugins/<package name>/zeusdl_plugins/`
2. **Executable location**: Plugin packages can similarly be installed in a `ZeusDL-plugins` directory under the executable location (recommended for portable installations):
    * Binary: where `<root-dir>/ZeusDL.exe`, `<root-dir>/ZeusDL-plugins/<package name>/zeusdl_plugins/`
    * Source: where `<root-dir>/zeusdl/__main__.py`, `<root-dir>/ZeusDL-plugins/<package name>/zeusdl_plugins/`

3. **pip and other locations in `PYTHONPATH`**
    * Plugin packages can be installed and managed using `pip`. See [ZeusDL-sample-plugins](https://github.com/ZeusDL/ZeusDL-sample-plugins) for an example.
      * Note: plugin files between plugin packages installed with pip must have unique filenames.
    * Any path in `PYTHONPATH` is searched in for the `zeusdl_plugins` namespace folder.
      * Note: This does not apply for Pyinstaller builds.


`.zip`, `.egg` and `.whl` archives containing a `zeusdl_plugins` namespace folder in their root are also supported as plugin packages.

* e.g. `${XDG_CONFIG_HOME}/ZeusDL/plugins/mypluginpkg.zip` where `mypluginpkg.zip` contains `zeusdl_plugins/<type>/myplugin.py`

Run ZeusDL with `--verbose` to check if the plugin has been loaded.

## Developing Plugins

See the [ZeusDL-sample-plugins](https://github.com/ZeusDL/ZeusDL-sample-plugins) repo for a template plugin package and the [Plugin Development](https://github.com/ZeusDL/ZeusDL/wiki/Plugin-Development) section of the wiki for a plugin development guide.
...
If you are a plugin author, add [ZeusDL-plugins](https://github.com/topics/ZeusDL-plugins) as a topic to your repository for discoverability.

See the [Developer Instructions](https://github.com/ZeusDL/ZeusDL/blob/master/CONTRIBUTING.md#developer-instructions) on how to write and test an extractor.

# EMBEDDING ZeusDL

ZeusDL makes the best effort to be a good command-line program, and thus should be callable from any programming language.

Your program should avoid parsing the normal stdout since they may change in future versions. Instead, they should use options such as `-J`, `--print`, `--progress-template`, `--exec` etc to create console output that you can reliably reproduce and parse.

From a Python program, you can embed ZeusDL in a more powerful fashion, like this:

```python
from zeusdl import YoutubeDL
...
Most likely, you'll want to use various options. For a list of options available, have a look at [`zeusdl/YoutubeDL.py`](zeusdl/YoutubeDL.py#L183) or `help(zeusdl.YoutubeDL)` in a Python shell. If you are already familiar with the CLI, you can use [`devscripts/cli_to_api.py`](https://github.com/ZeusDL/ZeusDL/blob/master/devscripts/cli_to_api.py) to translate any CLI switches to `YoutubeDL` params.

**Tip**: If you are porting your code from youtube-dl to ZeusDL, one important point to look out for is that we do not guarantee the return value of `YoutubeDL.extract_info` to be json serializable, or even be a dictionary. It will be dictionary-like, but if you want to ensure it is a serializable dictionary, pass it through `YoutubeDL.sanitize_info` as shown in the [example below](#extracting-information)
...
# ℹ️ See help(zeusdl.YoutubeDL) for a list of available options and public functions
ydl_opts = {}
with zeusdl.YoutubeDL(ydl_opts) as ydl:
...
ydl_opts = {
    ...
    # ℹ️ See help(zeusdl.postprocessor) for a list of available Postprocessors and their arguments
...
# ℹ️ See "progress_hooks" in help(zeusdl.YoutubeDL)
def my_hook(d):
...
# ℹ️ See help(zeusdl.postprocessor.PostProcessor)
class MyCustomPP(zeusdl.postprocessor.PostProcessor):
    def run(self, info):
...
with zeusdl.YoutubeDL() as ydl:
    # ℹ️ "when" can take any value in zeusdl.utils.POSTPROCESS_WHEN
...
* Forked from [**yt-dlc@f9401f2**](https://github.com/blackjack4494/yt-dlc/commit/f9401f2a91987068139c5f757b12fc711d4c0cee) and merged with [**youtube-dl@a08f2b7**](https://github.com/ytdl-org/youtube-dl/commit/a08f2b7e4567cdc50c0614ee0a4ffdff49b8b6e6) ([exceptions](https://github.com/ZeusDL/ZeusDL/issues/21))

* **[SponsorBlock Integration](#sponsorblock-options)**: You can mark/remove sponsor sections in YouTube videos by utilizing the [SponsorBlock](https://sponsor.ajay.app) API

* **[Format Sorting](#sorting-formats)**: The default format sorting options have been changed so that higher resolution and better codecs will be now preferred instead of simply using larger bitrate. Furthermore, you can now specify the sort order using `-S`. This allows for much easier format selection than what is possible by simply using `--format` ([examples](#format-selection-examples))

* **Merged with animelover1984/youtube-dl**: You get most of the features and improvements from [animelover1984/youtube-dl](https://github.com/animelover1984/youtube-dl) including `--write-comments`, `BiliBiliSearch`, `BilibiliChannel`, Embedding thumbnail in mp4/ogg/opus, playlist infojson etc. See [#31](https://github.com/ZeusDL/ZeusDL/pull/31) for details.
...
* **Subtitle extraction from manifests**: Subtitles can be extracted from streaming media manifests. See [commit/be6202f](https://github.com/ZeusDL/ZeusDL/commit/be6202f12b97858b9d716e608394b51065d0419f) for details
...
* **Self updater**: The releases can be updated using `ZeusDL -U`, and downgraded using `--update-to` if required

* **Automated builds**: [Nightly/master builds](#update-channels) can be used with `--update-to nightly` and `--update-to master`

See [changelog](Changelog.md) or [commits](https://github.com/ZeusDL/ZeusDL/commits) for the full list of changes

Features marked with a **\*** have been back-ported to youtube-dl

### Differences in default behavior

Some of ZeusDL's default options are different from that of youtube-dl and youtube-dlc:

* ZeusDL supports only [Python 3.10+](## "Windows 8"), and will remove support for more versions as they [become EOL](https://devguide.python.org/versions/#python-release-cycle); while [youtube-dl still supports Python 2.6+ and 3.2+](https://github.com/ytdl-org/youtube-dl/issues/30568#issue-1118238743)
* The options `--auto-number` (`-A`), `--title` (`-t`) and `--literal` (`-l`), no longer work. See [removed options](#Removed) for details
* `avconv` is not supported as an alternative to `ffmpeg`
* ZeusDL stores config files in slightly different locations to youtube-dl. See [CONFIGURATION](#configuration) for a list of correct locations
* The default [output template](#output-template) is `%(title)s [%(id)s].%(ext)s`. There is no real reason for this change. This was changed before ZeusDL was ever made public and now there are no plans to change it back to `%(title)s-%(id)s.%(ext)s`. Instead, you may use `--compat-options filename`
* The default [format sorting](#sorting-formats) is different from youtube-dl and prefers higher resolution and better codecs rather than higher bitrates. You can use the `--format-sort` option to change this to any order you prefer, or use `--compat-options format-sort` to use youtube-dl's sorting order. Older versions of ZeusDL preferred VP9 due to its broader compatibility; you can use `--compat-options prefer-vp9-sort` to revert to that format sorting preference. These two compat options cannot be used together
* The default format selector is `bv*+ba/b`. This means that if a combined video + audio format that is better than the best video-only format is found, the former will be preferred. Use `-f bv+ba/b` or `--compat-options format-spec` to revert this
* Unlike youtube-dlc, ZeusDL does not allow merging multiple audio/video streams into one file by default (since this conflicts with the use of `-f bv*+ba`). If needed, this feature must be enabled using `--audio-multistreams` and `--video-multistreams`. You can also use `--compat-options multistreams` to enable both
...
* `playlist_index` behaves differently when used with options like `--playlist-reverse` and `--playlist-items`. See [#302](https://github.com/ZeusDL/ZeusDL/issues/302) for details. You can use `--compat-options playlist-index` if you want to keep the earlier behavior
...
* When `--embed-subs` and `--write-subs` are used together, the subtitles are written to disk and also embedded in the media file. You can use just `--embed-subs` to embed the subs and automatically delete the separate file. See [#630 (comment)](https://github.com/ZeusDL/ZeusDL/issues/630#issuecomment-893659460) for more info. `--compat-options no-keep-subs` can be used to revert this
* `certifi` will be used for SSL root certificates, if installed. If you want to use system certificates (e.g. self-signed), use `--compat-options no-certifi`
* ZeusDL's sanitization of invalid characters in filenames is different/smarter than in youtube-dl. You can use `--compat-options filename-sanitization` to revert to youtube-dl's behavior
* ~~ZeusDL tries to parse the external downloader outputs into the standard progress output if possible (Currently implemented: [aria2c](https://github.com/ZeusDL/ZeusDL/issues/5931)). You can use `--compat-options no-external-downloader-progress` to get the downloader output as-is~~
* ZeusDL versions between 2021.09.01 and 2023.01.02 applies `--match-filters` to nested playlists. This was an unintentional side-effect of [8f18ac](https://github.com/ZeusDL/ZeusDL/commit/8f18aca8717bb0dd49054555af8d386e5eda3a88) and is fixed in [d7b460](https://github.com/ZeusDL/ZeusDL/commit/d7b460d0e5fc710950582baed2e3fc616ed98a80). Use `--compat-options playlist-match-filter` to revert this
* ZeusDL versions between 2021.11.10 and 2023.06.21 estimated `filesize_approx` values for fragmented/manifest formats. This was added for convenience in [f2fe69](https://github.com/ZeusDL/ZeusDL/commit/f2fe69c7b0d208bdb1f6292b4ae92bc1e1a7444a), but was reverted in [0dff8e](https://github.com/ZeusDL/ZeusDL/commit/0dff8e4d1e6e9fb938f4256ea9af7d81f42fd54f) due to the potentially extreme inaccuracy of the estimated values. Use `--compat-options manifest-filesize-approx` to keep extracting the estimated values
* ZeusDL uses modern http client backends such as `requests`. Use `--compat-options prefer-legacy-http-handler` to prefer the legacy http handler (`urllib`) to be used for standard http requests.
* The sub-modules `swfinterp`, `casefold` are removed.
* Passing `--simulate` (or calling `extract_info` with `download=False`) no longer alters the default format selection. See [#9843](https://github.com/ZeusDL/ZeusDL/issues/9843) for details.
* ZeusDL no longer applies the server modified time to downloaded files by default. Use `--mtime` or `--compat-options mtime-by-default` to revert this.
...
Using one of the yearly compat option aliases will pin ZeusDL's default behavior to what it was at the *end* of that calendar year.

The following compat options restore vulnerable behavior from before security patches:

* `--compat-options allow-unsafe-ext`: Allow files with any extension (including unsafe ones) to be downloaded ([GHSA-79w7-vh3h-8g4j](<https://github.com/ZeusDL/ZeusDL/security/advisories/GHSA-79w7-vh3h-8g4j>))

    > :warning: Only use if a valid file download is rejected because its extension is detected as uncommon
    >
    > **This option can enable remote code execution! Consider [opening an issue](<https://github.com/ZeusDL/ZeusDL/issues/new/choose>) instead!**
...
These options may no longer work as intended
    ...
    --prefer-avconv                  avconv is not officially supported by ZeusDL (Alias: --no-prefer-ffmpeg)
...
# CONTRIBUTING
See [CONTRIBUTING.md](CONTRIBUTING.md#contributing-to-ZeusDL) for instructions on [Opening an Issue](CONTRIBUTING.md#opening-an-issue) and [Contributing code to the project](CONTRIBUTING.md#developer-instructions)

# WIKI
See the [Wiki](https://github.com/ZeusDL/ZeusDL/wiki) for more information
