...
def parseOpts(overrideArguments=None, ignore_config_files='if_override'):  # noqa: N803
    ...
    def add_config(label, path=None, func=None):
        ...
        if root.parse_known_args()[0].ignoreconfig:
            ...
            current_path = os.path.join(path, 'ZeusDL.conf')
...
class _YoutubeDLOptionParser(optparse.OptionParser):
    ...
    def __init__(self):
        super().__init__(
            prog='ZeusDL' if detect_variant() == 'source' else None,
            version=__version__,
            usage='%prog [OPTIONS] URL [URL...]',
            epilog='See full documentation at  https://github.com/ZeusDL/ZeusDL#readme',
    ...
    def format_option_help(self, formatter=None):
        ...
        description = formatter.format_description(
            'Predefined aliases for convenience and ease of use. Note that future versions of ZeusDL '
...
def create_parser():
    ...
    general.add_option(
        ...
        help=(
            ...
            'Use the value "auto" to let ZeusDL guess ("auto_warning" to emit a warning when guessing). '
    ...
    general.add_option(
        ...
        default=[],
        help=(
            'Remote components to allow ZeusDL to fetch when required. '
            'This option is currently not needed if you are using an official executable '
            'or have the requisite version of the ZeusDL-ejs package installed. '
            ...
            'ejs:github (external JavaScript components from ZeusDL-ejs GitHub). '
    ...
    general.add_option(
        ...
        callback_kwargs={
            ...
            'configurations by reverting some of the changes made in ZeusDL. '
    ...
    authentication.add_option(
        ...
        help='Account password. If this option is left out, ZeusDL will ask interactively')
    authentication.add_option(
        '-2', '--twofactor',
    ...
    authentication.add_option(
        ...
        help='Multiple-system operator account password. If this option is left out, ZeusDL will ask interactively')
    authentication.add_option(
        '--ap-list-mso',
    ...
    authentication.add_option(
        ...
        help='Password for client certificate private key, if encrypted. '
             'If not provided, and the key is encrypted, ZeusDL will ask interactively')
    ...
    filesystem.add_option(
        ...
        help=(
            'Location in the filesystem where ZeusDL can store some downloaded information '
            '(such as client ids and signatures) permanently. By default ${XDG_CACHE_HOME}/ZeusDL'))
    filesystem.add_option(
    ...
    zeus_engine = optparse.OptionGroup(parser, 'ZeusDL Engine Options')
    zeus_engine.add_option(
        '--extract-json',
        action='store_true', dest='extract_json', default=False,
        help=(
            'Extract video information and output as clean structured JSON. '
            'No logs or extra text are written to stdout. '
            'On failure, outputs a structured error JSON'))
    zeus_engine.add_option(
        '--fast',
        action='store_true', dest='fast_mode', default=False,
        help=(
            'Fast extraction mode: skip unnecessary metadata and return only '
            'essential fields (title, streams). Use with --extract-json'))
    zeus_engine.add_option(
        '--json-progress',
        action='store_true', dest='json_progress', default=False,
        help=(
            'Output download progress as one JSON object per line to stdout. '
            'Each line contains: id, progress, speed, eta, downloaded_bytes, total_bytes, status'))
    zeus_engine.add_option(
        '--no-interactive',
        action='store_true', dest='no_interactive', default=False,
        help='Disable all interactive prompts. Suitable for embedding in automated systems')
    zeus_engine.add_option(
        '--task-id',
        dest='task_id', metavar='ID', default=None,
        help=(
            'Attach a task identifier to all JSON output. '
            'If not provided, a random ID is auto-generated when using --extract-json or --json-progress'))
    zeus_engine.add_option(
        '--quality',
        dest='quality_filter', metavar='QUALITY', default=None,
        help=(
            'Select the stream closest to the given quality (e.g. 720p, 1080p). '
            'Falls back to the best available if the exact quality is not found'))
    zeus_engine.add_option(
        '--silent',
    ...
    parser.add_option_group(zeus_engine)
...
def _hide_login_info(opts):
    deprecation_warning(f'"{__name__}._hide_login_info" is deprecated and may be removed '
                        'in a future version. Use "zeusdl.utils.Config.hide_login_info" instead')
