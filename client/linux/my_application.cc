...
static void my_application_activate(GApplication* application) {
  ...
  if (use_header_bar) {
    ...
    gtk_header_bar_set_title(header_bar, "ZeusDL");
    gtk_header_bar_set_show_close_button(header_bar, TRUE);
    gtk_window_set_titlebar(window, GTK_WIDGET(header_bar));
  } else {
    gtk_window_set_title(window, "ZeusDL");
  }
...
static void my_application_local_command_line(GApplication* application,
  ...
  self->dart_entrypoint_arguments = g_new0(gchar*, n);
  int arg_index = 0;
  for (int i = 1; i < n; i++) {
    if (g_str_has_prefix(args[i], "--")) {
      self->dart_entrypoint_arguments[arg_index++] = g_strdup(args[i] + 2);
    }
  }
...
MyApplication* my_application_new() {
  return MY_APPLICATION(g_object_new(my_application_get_type(),
                                     "application-id", "com.zeus.client",
                                     "flags",
